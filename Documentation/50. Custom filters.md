...
The filter class is responsible for applying the actual query constraint. It must implement the `FilterInterface` (or extend the provided `AbstractFilter`).
...
## 5. Registering the Filter

To make the Anthology extension aware of your new filter, you need to register it.

### Register the Filter Type

First, you need to add your filter to the list of available filter types. This is done in your extension's TypoScript.
...
plugin.tx_llanthology {
	settings {
		filterImplementations {
			tag = Vendor\MyExtension\Domain\Filter\TagFilter
		}
	}
}
```
...
Next, you need to associate the `tag` filter type with its FlexForm file. This is done in the TCA for the filter record. If no further configuration options are required, this is not necessary.
